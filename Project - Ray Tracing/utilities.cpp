...
double areaOfTriangle(double a, double b, double c) {
    ...
    if (a > 0 && b > 0 && c > 0) {
        double peri = (a + b + c) / 2;
        double result = glm::sqrt(peri * (peri - a) * (peri - b) * (peri - c));
        if (!glm::isnan(result) && !glm::isinf(result))
            return result;
    }
	return -1;
}
...
double directionInRadians(const dvec2& referencePt, const dvec2& targetPt) {
	/* CSE 386 - todo  */
    double result = glm::atan(targetPt.y - referencePt.y, targetPt.x - referencePt.x);
    if (result >= 0)
        return result;
    return (2 * PI) + result;
}
...
double directionInRadians(const dvec2& targetPt) {
	/* CSE 386 - todo  */
    double result = glm::atan(targetPt.y, targetPt.x);
    if (result >= 0)
        return result;
    return (2 * PI) + result;
}
...
double directionInRadians(double x1, double y1, double x2,  double y2) {
	/* CSE 386 - todo  */
    double result = glm::atan(y2 - y1, x2 - x1);
    if (result >= 0)
        return result;
    return (2 * PI) + result;
}
...
bool isOrthogonal(const dvec3& a, const dvec3& b) {
    ...
    return approximatelyZero(dotProduct);
}
...
dmat3 multiplyMatrices(const vector<dmat3>& M) {
    ...
    for (int i = 1; i < M.size(); i++) {
        ...
        for (int j = 0; j < 3; j++) {
            for (int k = 0; k < 3; k++) {
                result[k][j] = glm::dot(getRow(matrix1, j), getCol(matrix2, k));
            }
        }
        matrix1 = result;
    }
