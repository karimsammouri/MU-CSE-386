...
color specularColor(const color& mat, const color& lightColor,
	...
	const dvec3& r, const dvec3& v) {
	/* CSE 386 - todo  */
    return glm::clamp(mat * lightColor * glm::pow(glm::max(0.0, glm::dot(r, v)), shininess), 0.0, 1.0);
}
...
color totalColor(const Material& mat, const color& lightColor,
    ...
    double AT = 1.0;
    if (attenuationOn == true) {
        AT = 1.0 / (ATparams.constant + (ATparams.linear * d) +
                    (ATparams.quadratic * glm::pow(d, 2.0)));
    }
    return glm::clamp(ambientColor(mat.ambient, lightColor) + (AT *
    (diffuseColor(mat.diffuse, lightColor, l, n) +
    specularColor(mat.specular, lightColor, mat.shininess, r, v))), 0.0, 1.0);
}
...
color PositionalLight::illuminate(const dvec3& interceptWorldCoords,
    ...
    if (isOn == false) {
        return black;
    } else if (isOn == true && inShadow == true) {
...
bool PositionalLight::pointIsInAShadow(const dvec3& intercept,
    ...
    for (int i = 0; i < objects.size(); i++) {
        ...
        double intersectionDistance = glm::distance(intercept, hit.interceptPt);
        double lightDistance = glm::distance(intercept, pos);
        if (hit.t != FLT_MAX && intersectionDistance < lightDistance) {
            return true;
        }
    }
	return false;
...
//	dvec3 origin(0, 0, 0);
//	dvec3 dir(1, 1, 1);
    dvec3 dir = pos - interceptWorldCoords;
